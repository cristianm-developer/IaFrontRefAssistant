...
A classic `<script>` global build is also published at `dist/ia-front-ref-assistant.global.js` for zero-bundler setups — it exposes the same two functions under `window.IaFrontRefAssistant`. See [INTEGRATION.md](INTEGRATION.md#plain-html-no-bundler).
