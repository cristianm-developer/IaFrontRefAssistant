...
> URL del paquete (git): `git+https://github.com/TU_USUARIO/ia-front-ref-assistant.git`
> Reemplazá `TU_USUARIO` por el owner real una vez que el repo esté pusheado
> a GitHub (ver `package.json` del paquete, campo `repository.url`, que
> debe tener la misma URL). Si este comando se corre desde una copia local
...
2. Verificá que `ia-front-ref-assistant` esté en las dependencias de
   ...
   - `npm`: `npm install <URL del paquete de arriba>`
   - `pnpm`: `pnpm add <URL del paquete de arriba>`
   - `yarn`: `yarn add <URL del paquete de arriba>`

   El paquete tiene un script `prepare` que corre `npm run build` (Vite)
   automáticamente al instalarse desde git, así que no hace falta ningún
   paso extra para que `dist/` exista — el propio gestor de paquetes lo
   genera durante el install. Si el install falla, mostrá el error tal
   cual y no sigas (no tagees ni montes nada sobre un paquete que no
   quedó instalado).
